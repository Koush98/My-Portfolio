...
const typed = new Typed('.multiple-text', {
  strings: ['Data Analyst', 'Business Analyst', 'Machine Learning Engineer'],
  typeSpeed: 100,
  backSpeed: 100,
  backDelay: 1000,
  loop: true
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function showNextCertificate() {
    certificates.forEach((certificate, i) => {
      certificate.style.transform = `translateX(-${index * 100}%)`;
    });
    index = (index + 1) % certificates.length;
  }

  setInterval(showNextCertificate, 3000); // Change certificate every 3 seconds
});